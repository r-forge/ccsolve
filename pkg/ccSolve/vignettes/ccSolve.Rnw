...
in R, more specifically differential equations, root solving problems,
optimization and least squares problems. It works with solvers from the 
...
and \code{uniroot} from \pkg{R-base} \citep{R2014} and to function \code{nls.lm} 
...
up to 50 times over problems specified in R-code. However, often the speed gain is just 
a factor two to an order of magnitude, while in certain cases the speed gain may be even negligible.

The problem also needs to be compiled before it can be solved; this often takes a few seconds;
and this needs to be taken into account before deciding to solve via compiled code.
So, these functions may not provide a good alternative to R-code for one-time use.

However, there are functions to save and load compiled problems, so compilation
needs to be done only once and
\cc may prove worthwhile for certain analyses that need to be repeated multiple 
times.
}
...
\rs \citep{rootSolve}, \pkg{deTestSet} \citep{deTestSet} and \pkg{minpack.lm} \citep{minpack.lm}. 
...
differential equations (PDE), of functions that solve for the root of (multiple) or single
...
function wrapper written. This object can then be used as argument in the 
associated solver.

The package comprizes:
  \begin{itemize}
    \item function \code{compile.ode} to create compiled code of initial value ordinary 
    ...
    \item function \code{compile.multiroot} to compile root finding problems, from the 
    R-package \pkg{rootSolve}.
    \item function \code{compile.optim} and \code{compile.uniroot} to compile one dimensional 
     optimization  and root finding problems. 
  \end{itemize}

The solver packages (\ds, \bvp, \rs, and \pkg{deTestSet}) already include the 
facility to write problems in compiled code; they have been extended to also 
accept compiled code generated by \cc \footnote{so you may need to update your version}.

The best description of how R solvers can be used in conjunction with compiled code
is in the \pkg{deSolve} vignette (`compiledCode') \citep{compiledCode}.
it is clear from this vignette that this is a rather technical endeavour; 
it requires problem codes to be written 
in separate files and obeying strict rules, that are then compiled, linked and loaded. 

In contrast, the new package \pkg{ccSolve} allows to define the problem as text strings, 
in R, and it takes care of the technical aspects the user has to go throught when 
...
that extend the \code{optim}, \code{uniroot} and \code{nls} functions to 
also support problems written in compiled code. To make these extensions,
the original formulations from the \pkg{R-base} software and from the R-packge \pkg{minpack.lm} 
were altered.

Depending on the problem itself, compiled functions may be up to 50 times faster
than R-functions, but in some cases the speed gain will be as small as a factor two. 
...
As an example, the arguments of \code{compile.bvp} are:
<<>>=
args(compile.bvp)
@
The functions that make compiled versions to be used with extensions of certain
numerical solvers from te R-base software are similar, i.e. for solving optimization
problems:
<<>>=
args(compile.optim)
@
...
root-solving problems (where \code{x} is a vector) is called  \code{compile.multiroot}. 
It extends the root solving functions of the R-package \pkg{rootSolve}, 
\code{multiroot} and \code{multiroot.1D}.

Single roots (\code{x} is one value) in compiled code are generated with 
function \code{compile.uniroot}, to be solved with \code{uniroot} from 
\pkg{R-base} or \code{uniroot.all} from \pkg{rootSolve}.

To compile problems to be used with the steady-state solvers, (that find the 
roots or steady-state solutions of differential equations), 
i.e. \code{stode} and \code{stodes} the function \code{compile.steady} should be used.

Functions \code{compile.multiroot} and \code{compile.uniroot} are called as:
<<>>=
args(compile.multiroot)
args(compile.uniroot)
@
\subsection{The root of one function}
<<>>=
...
we can print it using \code{printCode}.
<<>>=
...
Function \code{compile.multiroot} creates the common block (fortran) or global vector (C) and assigns the 
...
in the function that is generated with \code{compile.multiroot}. 
...
Optimization problems are usually small problems, involving few equations and
few iterations, or they are efficiently vectorised in R, 
so putting them in compiled code will not give large speed gains.

The \code{optim} function has been extended so that it also supports 
minimizing problems written in compiled code. 
The new function is called \code{ccoptim} and takes the same arguments 
as \code{optim}.
...
print(system.time(AA <- ccoptim (fn = cNonlin, par = ini, 
  method = "CG", control = list(maxit = 1000))))
@
There is less efficiency gain when the model is solved with 
the BFGS method:
<<>>=
print(system.time(A <- optim (fn = Nonlin.R, par = ini, 
  method = "BFGS")))
...
Here is another minimzation problem:
<<>>=
brown.f <- function(p) {
...
summary(fm2DNase1)
@
The problme is compiled such that the compiler knows the names of the 
parameters to be solved for 
and the names of the data; the values of parms and data can differ during
...
@
Now it is straighforward to use the fast model version to fit all the Runs:
<<>>=
for ( i in 1:11)
...
This relates to the solvers themselves.
\begin{itemize}
...
when calling the solver. They are unnamed, and can be used for input and output. 
For differential equation solvers, they are used to contain the output variables.
See vignette (`compiledCode') \citep{compiledCode}
\item \code{parms} is to contain the values of named variables, whose length is 
 known during compilation. They are declared in a common block (Fortran) or as global
 variables (C) and their value is set at the start of the solution procedure, as
 passed with argument \code{parms}, which is a (named) vector or list. 
 They are not supposed to be changed. Not implemented for the minmization, uniroot and 
 nonlinear lest squares methods (the name \code{parms} is too close to the official argument
 \code{par} which is completely different)
\item {forcings} This is only used for differential equations. It is to contain the  
variables that are updated by the solver, every time (or spatial) step.
\item \code{data}. This is to contain data in matrix or data.frame format, 
and whose length is not necessarily known at compile time. The names of the columns are 
used to set variable names in the code; the variables are declared in a module (Fortran) 
or as global variables (C). Their values are set at the start of the simulation.
