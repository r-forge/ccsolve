...
in \proglang{R}, more specifically differential equations, root solving problems,
...
However, using the functions to save and load compiled problems, compilation
...
The functions then compile the code, load the DLL or shared object, and add
a function wrapper. This resulting object can then be used as argument in the
associated solver.
...
The package comprizes:
  \begin{itemize}
   ...
   solving nonlinear least squares and optimization methods, to be used with extended versions
   of solvers for the \pkg{stats} \citep{R2014} package.
...
the original formulations from the \pkg{stats} package \citep{R2014} for R core,
and from the R-packge \pkg{minpack.lm} \citep{minpack.lm} were altered.

Writing compiled code is a rather technical endeavour: it requires problem
codes to be written in separate files, according to strict
...
on ideas from the R-package \pkg{inline} \citep{inline}; however, several additions
were necessary to make the functions of this package
suitable for use with \proglang{Fortran} or \pkg{F95}.
...
numerical solvers from R-base are similar, i.e. for solving optimization
problems:
<<>>=
args(compile.optim)
@
...
and the compiling functions will expand them, e.g. by adding the function or subroutine definition,
...
either create a common block or module (\proglang{Fortran}) or declare global variables
...
If we print the \proglang{F95} code, the wrapper written by \code{compile.uniroot}
is clear:
<<>>=
printCode(croot.f95)
printCode(croot.C)
...
A printout of the \proglang{F95} code shows this looks like for the current problem;
...
the jacobian which has nonzero values only in the diagonal and immediately
...
code instead.
Apart from the fortran versus R implementation, the method in the latter
...
are \code{conc} and \code{density}.
...
During compilation, only the names are important; the representative values will be passed
during the actual application; this is done in the added subroutine \code{initdat},
which will be called by the solver; in addition, \code{initdat} will call a solver
function (called \code{nlsdat}).
...
contain the output variables. See vignette (`compiledCode') \citep{compiledCode}
