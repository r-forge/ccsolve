...
\newcommand{\rs}{\pkg{rootSolve }\xspace}
 \title{
Package \pkg{ccSolve}: solving numerical problems in compiled code.
}
...
The package comprizes:
  \begin{itemize}
    ...
    \item function \code{compile.optim} to compile optimization problems. 
     \pkg{ccSolve} also contains a function called \code{ccoptim} that extends 
     the \code{optim} function to also support compiled code.
  \end{itemize}
...
This needs to be taken into account when deciding whether the route to formulate
...
few iterations, so
putting them in compiled code will not give large speed gains.

Nevertheless, we extended the \code{optim} function so that it also supports 
